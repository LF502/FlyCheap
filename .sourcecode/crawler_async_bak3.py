...
async def getTickets(fdate: datetime.date, days: int, dcity: str, acity: str, path: Path, values_only: bool = False) -> tuple():
    "Get tickets from dep city to arr city of collecting days, return tuple of dcity and acity."
    ...
    for day in range(days):
        ...
        payload['airportParams'][0]['date'] = fdate.isoformat()
        print(dcity, acity, fdate.isoformat(), 'processing...')
        for retry in range(3):
            try:
                header['User-Agent'] = userAgents[int(len(userAgents) * random())]
                connector = TCPConnector(limit = 512, ssl = False, force_close = True, use_dns_cache = False)  
...
async def generateXlsx(path: Path, fdate: datetime.date, days: int = 10, day_limit: int = 0, codeList: list = ['BJS','CAN'], 
    ...
    total = (codesum * (codesum - 1) - len(skipSet)) / 2
    ...
    for dcity in range(codesum):
        for acity in range(dcity,codesum):
            ...
            if (dcity,acity) not in skipSet: # If the city tuple key / coordinate is not found, process.
                tasks.append(asyncio.ensure_future(getTickets(fdate, days, codeList[dcity], codeList[acity], path, values_only)))
                tasks.append(asyncio.ensure_future(getTickets(fdate, days, codeList[acity], codeList[dcity], path, values_only)))
...
if __name__ == "__main__":
    ...
    #loop.run_until_complete(generateXlsx(path, datetime.date(2022,2,15), 10, 0, ['NKG','CKG','TSN'], ignore_cities, ignore_threshold, True))    #测试用例
    loop.close