...
class CtripCrawler:
    ...
    def __init__(self, cityList: list, flightDate: datetime.date = datetime.datetime.now().date(), days: int = 1, day_limit: int = 0, 
        ...
        self.__idct = 0
        self.__avgTime = 4 if with_return else 1.8
    ...
    def proxy(self) -> dict | None:
        ...
        try:
            ...
            time.sleep((round(3 * random.random(), 2)))
        finally:
            return proxy
    ...
    @staticmethod
    def sign(transaction_id, dcity: str, acity: str, flightDate: str):
        sign_value = transaction_id + dcity + acity + flightDate
        _sign = hashlib.md5()
        _sign.update(sign_value.encode('utf-8'))
        return _sign.hexdigest()


    def transaction_id(self, dcity: str, acity: str, flightDate: str, proxy: dict = None):
        url = "https://flights.ctrip.com/international/search/api/flightlist" \
                        "/oneway-{}-{}?_=1&depdate={}" \
            .format(dcity, acity, flightDate)
    ...
    def collector(self, flightDate: datetime.date, dcity: str, acity: str) -> list():
        ...
        proxy = self.proxy
        transaction_id, data = self.transaction_id(dcity, acity, flightDate.isoformat(), proxy)
        if transaction_id == "" or data is None:
            return datarows
        header = {"origin": "https://flights.ctrip.com", 
                  "content-type": "application/json;charset=UTF-8", 
                  "User-Agent": self.__userAgents[random.randrange(0, self.__lenAgents)], 
                  "transactionid": transaction_id, 
                  "sign": self.sign(transaction_id, dcity, acity, flightDate.isoformat()), 
                  "scope": data["scope"], 
                  "referer": "https://flights.ctrip.com/online/list/oneway-{}-{}?_=1&depdate={}".format(dcity, acity, flightDate.isoformat()), 
                  "cookie": self.cookie, }
        ...
        try:
            for route in routeList:
                ...
                if len(flightSegments) == 1:    # Flights that need to transfer is ignored.
                    ...
                    airlineName = flight.get('marketAirlineCode')
    ...
    def output_excel(datarows: list, dcity: str, acity: str, path: Path = Path(), values_only: bool = False, with_return: bool = True) -> bool:
        ...
        if values_only:
            ...
            wsheet.column_dimensions['C'].width = wsheet.column_dimensions['J'].width = 6
            wsheet.column_dimensions['D'].width = wsheet.column_dimensions['I'].width = 6
